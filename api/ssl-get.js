...
export default router.post('/ssl-get', async (req, res) => {
   
  const { amount, serviceId ,user_id} =  req.query;  
  ...
  try {
    if(amount  && user_id){
        const data = {
            ...
            currency: 'BDT',
            tran_id: tran_id ,  
            success_url: `http://localhost:4000/api/ssl/success?session_id=${tran_id}&amount=${amount}&user_id=${user_id}&service_id=${serviceId}`,
